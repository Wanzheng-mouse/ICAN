    ...
    "properties": {
        ...
        "profile": {
            ...
            "properties": {
                ...
                "warehouse_area_m2": {"type": ["integer", "null"]},
                "daily_orders": {"type": ["integer", "null"]},
                "peak_orders_per_hour": {"type": ["integer", "null"]},
                "sku_count": {"type": ["integer", "null"]},
                "tote_agv_count": {"type": ["integer", "null"]},
                "pallet_agv_count": {"type": ["integer", "null"]},
                "agv_count": {"type": ["integer", "null"]},
                "robotic_arm_count": {"type": ["integer", "null"]},
                "pick_station_count": {"type": ["integer", "null"]},
                ...
                "targets": {
                    ...
                    "additionalProperties": False,
                    "properties": {
                        "order_completion_rate": {"type": ["integer", "null"]},
                        "average_wait_seconds": {"type": ["integer", "null"]},
                        "device_utilization_rate": {"type": ["integer", "null"]},
                    },
                    "required": [
        ...
        "operational_design": {
            ...
            "additionalProperties": False,
            "properties": {
                "traffic_policy": {"type": "string"},
                "collision_policy": {"type": "string"},
                "charging_policy": {"type": "string"},
        ...
        "candidate_guidance": {
            ...
            "items": {
                ...
                "properties": {
                    "strategy": {
                        "type": "string",
                        "enum": ["balanced", "throughput", "energy_saver"],
                    },
                    "title": {"type": "string"},
...
def analyze_with_agnes(
    requirement: str, sources: list[dict[str, Any]], settings: Settings
) -> dict[str, Any]:
    """Use Agnes' Chat Completions-compatible endpoint for server-side analysis."""
    if not settings.agnes_api_key:
        raise HTTPException(
            status_code=503,
            detail="LLM_ANALYSIS_NOT_CONFIGURED: set ICAN_AGNES_API_KEY in services/api/.env and restart the backend.",
        )
    files = (
        "\n".join(
            f"- {item.get('kind', 'file')}: {item.get('name', 'unnamed')}" for item in sources
        )
    ...
    body = {
        ...
        "response_format": {
            "type": "json_schema",
            "json_schema": {
                "name": "warehouse_requirement_analysis",
                "strict": True,
    ...
    try:
        ...
        raise HTTPException(
            status_code=502, detail=f"LLM_ANALYSIS_UPSTREAM_UNAVAILABLE: {exc}"
        ) from exc
    if response.status_code >= 400:
        raise HTTPException(
            status_code=502,
            detail=f"LLM_ANALYSIS_UPSTREAM_ERROR: {response.status_code} {response.text[:300]}",
        )
    ...
    if finish_reason == "length":
        raise HTTPException(
            status_code=502,
            detail="LLM_ANALYSIS_OUTPUT_TRUNCATED: increase ICAN_LLM_MAX_TOKENS or request a shorter response.",
        )
    try:
        result = json.loads(_output_text(response_data))
    except (ValueError, TypeError) as exc:
        raise HTTPException(
            status_code=502, detail="LLM_ANALYSIS_INVALID_STRUCTURED_OUTPUT"
        ) from exc
    ...
    strategies = [
        item.get("strategy")
        for item in result.get("candidate_guidance", [])
        if isinstance(item, dict)
    ]
    if strategies != ["balanced", "throughput", "energy_saver"]:
        raise HTTPException(
            status_code=502,
            detail="LLM_ANALYSIS_SCHEMA_VALIDATION_FAILED: candidate strategies must be balanced, throughput, energy_saver in order.",
        )
