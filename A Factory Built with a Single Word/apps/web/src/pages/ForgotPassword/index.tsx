...
export default function ForgotPasswordPage() {
  ...
  const handleFinish = async (values: { email: string }) => {
    message.loading({ content: '模拟发送中…', key: 'reset', duration: 1.5 });
    await new Promise((r) => setTimeout(r, 1500));
    message.success({ content: `（演示环境）密码重置链接已发送至 ${values.email}`, key: 'reset', duration: 6 });
    form.resetFields();
  };
        ...
        <Form form={form} onFinish={handleFinish} layout="vertical" size="large" className="login-form">
          ...
          <div className="reset-hint">
            <Text type="secondary" style={{ fontSize: 12 }}>
              ⚡ 演示环境说明：当前为前端 Mock 阶段，不会实际发送邮件。
              后端邮箱服务就绪后，此处将真实调用 <code>/api/v1/auth/reset-password</code> 接口。
            </Text>
          </div>
          <Form.Item>
